...
function getFileSystemSuccess(fs) {
  ...
  theFileSystem = fs;
  //Kick off a refresh the file list
  refreshFileList();
...
function getFileSuccess(theEntry) {
  ...
  console.log("Creating continuous file writer");
  var cFileWriter = new ContinuousFileWriter(theEntry);
  cFileWriter.writeLine("Apache Cordova API Cookbook" + br);
  cFileWriter.writeLine("File created by Example 10.1" + br);
  cFileWriter.writeLine("This is line of text" + br);
  cFileWriter.writeLine(createRandomStory(25));
